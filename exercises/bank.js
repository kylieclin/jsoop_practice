...
class Bank{
	...
	makeAccount(number){
		...
		if(this.accounts.hasOwnProperty(number)){
			return false;
		} else {
			this.accounts[number] = new Account(); ///important!!! abc123 becomes new Account();
	...
	withdraw( accountNumber, amount ){
		...
		if(!this.accounts.hasOwnProperty(accountNumber)){
			...
			var removeamount = this.accounts[accountNumber].remove(amount); //still called the function

			return "removed " + removeamount + ' from account ' + accountNumber + ". It now has " + this.accounts[accountNumber].getAmount();		
		}
